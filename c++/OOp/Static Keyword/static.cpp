...
class Test {
   ...
   public:
    ...
    // getter function for accessing the static variable (static member function)
    static int accessStatic() { return total_customer; }
    void Display() {
        cout << name << " " << acc << " " << balance << " " << total_customer << endl;
    }
    void DisplayTotal() { cout << total_customer << endl; }
};
